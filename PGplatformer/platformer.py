...
def draw_stats(player, missile_limit):
    ...
    pos=17
    player_quan = player.health//5
    for x in range(20):
        if player_quan > 0:
            pygame.draw.line(DISPLAY, c_light_blue, (pos, 8), (pos, 33), 3)
            player_quan-=1
        else:
            pygame.draw.line(DISPLAY, c_lighter_blue, (pos, 8), (pos, 33), 3)
        pos+=4
    pos = WIDTH/4+24
    player_quan = player.ammo
    for x in range(3):
        if player_quan > 0:
            pygame.draw.line(DISPLAY, c_orange, (pos, 8), (pos, 33), 23)
            player_quan-=1
        else:
            pygame.draw.line(DISPLAY, c_light_oragne, (pos, 8), (pos, 33), 23)
        pos+=28
...
while True:
    ...
    while True:
        ...
        if current_time - enemy_fired_time >= config.ENEMY_MISSILE_COOL:                                                        # enemy fire
        ...
        if player.rect.top > HEIGHT or player.health <= 0:  # game over
            ...
            time.sleep(2)
            pygame.mixer.music.load(config.SE_DIR + config.GAME_MSOUNDTRACK)
            pygame.mixer.music.play(-1)
            config.main_menu.enable()
            break
