...
/*
double Processing::singleWABprocessing2(const double* inputNeurons, const double* inputWeights, const double& inputbias, const int& size)
{
	//std::cout << "inputNeurons size: " << inputNeurons.size() << std::endl;//temp<<<<<<
	//std::cout << "inputWeights size: " << inputWeights.size() << std::endl;//temp<<<<<<
	//std::cout << "000";//temp<<<<<<
	...
	double holdNeuron = 0;
	for (int i = 0; i < size; i++) holdNeuron += inputWeights[i] * inputNeurons[i];
	//for (int i = 0; i < 784; i++) holdNeuron += inputWeights[i] * inputNeurons[i];
	//std::cout << "111";//temp<<<<<<
	return holdNeuron + inputbias;
}
